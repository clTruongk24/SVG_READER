...
// Clear cached bitmap and key (if any was used)
void RadialGradient::clearCache() {
	if (cachedBitmap) {
		delete cachedBitmap;
		cachedBitmap = nullptr;
	}
...
// Build a cache key string from bounds and radial parameters
string RadialGradient::generateCacheKey(const RectF& bounds,
	float centerX, float centerY,
	float radius, float focalX, float focalY) const {

	char buffer[256];
	sprintf_s(buffer, "%.1f_%.1f_%.1f_%.1f_%.1f_%.1f_%.1f_%.1f_%d",
...
// Approximate radial gradient using concentric rings filled with SolidBrush.
//  - Useful as a faster fallback; keeps a cached bitmap to avoid re-render.
Brush* RadialGradient::createApproximateRadialGradient(
    const RectF& bounds,
    float centerX, float centerY,
    float radius,
    ...
    if (radius <= 0.001f) {
        Color col = stops.empty() ? Color::Black : stops.back().getColor();
        BYTE alpha = stops.empty() ? 255 : static_cast<BYTE>(stops.back().getOpacity() * 255);
        return new SolidBrush(Color(alpha, col.GetR(), col.GetG(), col.GetB()));
    }

    int w = (int)ceil(bounds.Width) + 4;
    int h = (int)ceil(bounds.Height) + 4;

    const int MAX_DIMENSION = 2048;
    if (w > MAX_DIMENSION || h > MAX_DIMENSION) {
        float scale = min((float)MAX_DIMENSION / w, (float)MAX_DIMENSION / h);
        w = (int)(w * scale);
        h = (int)(h * scale);
    }
    if (w <= 0 || h <= 0) {
    ...
    string currentKey = generateCacheKey(bounds, centerX, centerY, radius, focalX, focalY);

    bool needsRender = false;
    if (!cachedBitmap || cacheKey != currentKey ||
        cachedBitmap->GetWidth() != w || cachedBitmap->GetHeight() != h) {
        if (cachedBitmap) {
            delete cachedBitmap;
            cachedBitmap = nullptr;
        }
        cachedBitmap = new Bitmap(w, h, PixelFormat32bppARGB);
        cacheKey = currentKey;
        needsRender = true;
    }

    // Render rings to bitmap if needed
    if (needsRender) {
        Graphics g(cachedBitmap);
        g.SetSmoothingMode(SmoothingModeAntiAlias);
        g.SetCompositingQuality(CompositingQualityHighSpeed);
        g.Clear(Color::Transparent);

        int numRings;
        if (radius < 50)      numRings = 30;
        else if (radius < 200) numRings = 50;
        else if (radius < 500) numRings = 80;
        else                   numRings = 100;

        float localCenterX = centerX - bounds.X;
        float localCenterY = centerY - bounds.Y;

        float focalDist = 0.0f;
        if (hasFocal) {
            focalDist = sqrt((centerX - focalX) * (centerX - focalX) +
                             (centerY - focalY) * (centerY - focalY));
        }

        // Fill concentric rings with colors from stops
        for (int i = numRings; i >= 0; --i) {
            ...
            float offset = t;
            if (hasFocal && focalDist > 0.001f && radius > 0.001f) {
                offset = (currentRadius - focalDist) / radius;
                if (spreadMethod == "pad") {
            ...
            Color color = getColorAtOffset(offset);

            SolidBrush ringBrush(color);
            g.FillEllipse(&ringBrush,
                localCenterX - currentRadius,
                localCenterY - currentRadius,
                currentRadius * 2.0f,
                currentRadius * 2.0f);
        }
    }

    // Use the cached bitmap directly; caller owns the returned brush.
    TextureBrush* textureBrush = new TextureBrush(cachedBitmap, WrapModeClamp);
    textureBrush->TranslateTransform(bounds.X, bounds.Y);
