...
RectF group::getBounds() const
{
    ...
    RectF boundingBox = children[0]->getBounds();

    for (size_t i = 1; i < children.size(); ++i)
    {
        RectF childBounds = children[i]->getBounds();

        float left = min(boundingBox.GetLeft(), childBounds.GetLeft());
        float top = min(boundingBox.GetTop(), childBounds.GetTop());
        float right = max(boundingBox.GetRight(), childBounds.GetRight());
        float bottom = max(boundingBox.GetBottom(), childBounds.GetBottom());

        boundingBox = RectF(left, top, right - left, bottom - top);
    }
    return boundingBox;
}